...
import { 
  ...
  FiCalendar, FiShoppingBag, FiDownload, FiRotateCw
} from 'react-icons/fi';
...
const OrderDetails = () => {
  ...
  const handleReorder = async () => {
    try {
      const response = await orderService.reorderOrder(id);
      if (response.success) {
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            {(order.orderStatus === 'shipped' || order.orderStatus === 'delivered' || trackingInfo) && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                {loadingTracking ? (
                  <div className="text-center py-4">
                    ...
                    <p className="text-sm text-gray-600 mt-2">Loading tracking info...</p>
                  </div>
                ) : trackingInfo ? (
                  <div className="space-y-3">
                    {trackingInfo.trackingNumber && (
                      <div>
                        ...
                        <p className="font-semibold text-lg">{trackingInfo.trackingNumber}</p>
                      </div>
                    )}
                  ...
                  <p className="text-gray-600 text-sm">Tracking information will be available once the order is shipped.</p>
                )}
              </motion.div>
            )}
            ...
            {order.isB2BOrder && (
              ...
              >
                <h2 className="text-xl font-bold text-secondary mb-4">B2B Order Details</h2>
                <div className="space-y-2 text-sm">
                  {order.purchaseOrderNumber && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">PO Number:</span>
                      <span className="font-semibold">{order.purchaseOrderNumber}</span>
                    </div>
                  )}
                  {order.paymentMethod && (
                    <div className="flex justify-between">
                  ...
                  {order.requiresApproval && (
                    <div className="mt-3 pt-3 border-t border-blue-300">
                      <p className="text-amber-700 text-sm">
                        ⚠️ This order requires admin approval
                      </p>
                    </div>
                  )}
                  {order.notes && (
            ...
            >
              ...
              <div className="space-y-2">
                <Link
                  to="/contact"
                  className="block text-center bg-primary text-white px-4 py-2 rounded-lg hover:bg-secondary transition-colors font-semibold text-sm"
                >
                  Contact Support
                </Link>
                <Link
                  to="/messages"
                  className="block text-center bg-white text-primary px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors font-semibold text-sm border border-primary"
                >
                  Send Message
                </Link>
              </div>
